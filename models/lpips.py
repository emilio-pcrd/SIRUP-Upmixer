...
class ScalingLayer(nn.Module):
    def __init__(self):
        ...
        self.view = lambda x: x.view(x.shape[0], x.shape[1], 128, 128)

    def forward(self, inp):
        inp = self.view(inp)
        assert inp.size(-2) == inp.size(-1)
        if inp.size(1) == 2:  # If the input has 2 channels, interpolate to 3 channels
